import static com.company.project.core.ProjectConstant.BASE_MODEL;
...
public class CodeGenerator {
    ...
    //JDBC配置，请修改为你项目的实际配置
    private static final String JDBC_URL = "jdbc:mysql://localhost:3319/db_lemonwms";
    ...
    private static final String PACKAGE_PATH_SERVICE = packageConvertPath(
        SERVICE_PACKAGE);//生成的Service存放路径
    private static final String PACKAGE_PATH_SERVICE_IMPL = packageConvertPath(
        SERVICE_IMPL_PACKAGE);//生成的Service实现存放路径
    private static final String PACKAGE_PATH_CONTROLLER = packageConvertPath(
        CONTROLLER_PACKAGE);//生成的Controller存放路径

    private static final String CLASS_DESC = "This guy is lazy, nothing left.";//@classDesc
    private static final String AUTHOR = "Auto Generator";//@author
     ...
     * 对所有数据表生成代码
     */
    private static void genCodeAll() {
        try {
            ...
            while (tables.next()) {
                genCode(tables.getString("TABLE_NAME"));
            }

        } catch (SQLException e) {
            System.out.println("数据库连接异常！");
        } catch (ClassNotFoundException e) {
            System.out.println(JDBC_DIVER_CLASS_NAME + " not found!");
        }
    }
...
*/
        ...
        javaModelGeneratorConfiguration.addProperty("rootClass", BASE_MODEL);
        ...
        if (generator.getGeneratedJavaFiles().isEmpty() || generator.getGeneratedXmlFiles()
            .isEmpty()) {
    ...
    private static void genService(String tableName, String modelName) {
        try {
            ...
            File file = new File(
                PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_SERVICE + modelNameUpperCamel
                    + "Service.java");
    ...
    private static void genController(String tableName, String modelName) {
        try {
            ...
            File file = new File(
                PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_CONTROLLER + modelNameUpperCamel
                    + "Controller.java");
            if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs();
            }
